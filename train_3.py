...
class LitDCAE(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        recon = self(batch)
        loss = F.mse_loss(recon.float(), batch.float())
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0):
        ...
        if step % self.save_every == 0 or step == cfg["num_steps"]:
            ...
            with torch.no_grad():
                ...
                gt = batch.float().cpu()
                mse_val = float(F.mse_loss(recon, gt).detach())
                ...
                tqdm.write(f"Step {step} MSE={mse_val:.2f} PSNR={psnr_val:.2f} dB Throughput={throughput:.2f} samples/s")

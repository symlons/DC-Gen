"""
GAN loss module for local refinement in diffusion-based generation.
Implements a patch-based discriminator with adversarial training.
"""
import torch
...
class PatchDiscriminator(nn.Module):
    """
    Patch-based discriminator for local refinement.
    Takes patches/regions and classifies them as real or fake.
    """
    def __init__(self, in_channels: int = 1, ndf: int = 64, n_layers: int = 3):
        """
        Args:
            in_channels: Number of input channels (1 for CT/medical images)
        ...
        # Output layer (1 channel = real/fake classification)
    ...
    def forward(self, real: torch.Tensor, fake: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            real: Real image patches [B, C, D, H, W]
            fake: Generated image patches [B, C, D, H, W]
            
        Returns:
            Logits for patch authenticity
        """
        # Concatenate real and fake along channel dimension
...
class GANLoss(nn.Module):
    """
    GAN loss with support for different training objectives.
    """
    def __init__(self, loss_type: str = "hinge"):
        """
        Args:
            loss_type: "hinge" (default) or "bce"
        """
        super().__init__()
        ...
        if loss_type == "bce":
            ...
            self.loss_fn = None  # Use hinge loss manually
        else:
    ...
    def discriminator_loss(
        ...
        Args:
            real_patches: Real image patches
            fake_patches: Generated image patches
            discriminator: Discriminator network
            
        Returns:
            Discriminator loss
        """
        # Forward pass — need separate logits for real and fake pairs
    ...
    def generator_loss(
        ...
        """
        Generator loss: make discriminator think fake is real.
        ...
        if self.loss_type == "bce":
            return self.loss_fn(fake_logits, torch.ones_like(fake_logits))
        else:  # hinge
...
class LocalPatchGAN:
    """
    Manager for patch-based GAN training.
    Handles extraction of local patches and adversarial loss computation.
    """
    def __init__(
        self,
        in_channels: int = 1,
        ...
        Args:
            ...
            patch_size: Size of patches for discriminator (D, H, W)
            ndf: Number of discriminator filters
    ...
    def extract_patch_pairs(
        ...
        """
        Extract spatially corresponding patches from real and fake volumes.
        
        Args:
            real: Real volume [B, C, D, H, W]
            fake: Fake volume [B, C, D, H, W]
            num_patches: Number of patches to extract per sample
            
        Returns:
            (real_patches, fake_patches) each [B*num_patches, C, pd, ph, pw]
        """
    ...
    def compute_discriminator_loss(
        self,
        real_volume: torch.Tensor,
        fake_volume: torch.Tensor,
    ) -> torch.Tensor:
        """
        Compute discriminator loss from volumes.
        
        Args:
            real_volume: Real image volume
            fake_volume: Generated image volume
        ...
        return self.gan_loss.discriminator_loss(
            real_patches, fake_patches, self.discriminator
        )
    ...
    def compute_generator_loss(
        ...
        """
        Compute generator adversarial loss from volumes.
        ...
        return self.gan_loss.generator_loss(
            real_patches, fake_patches, self.discriminator
        )

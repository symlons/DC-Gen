...
def save_volume_slices_triplet(gt_vol, recon_vol, path):
    ...
    for i in range(D):
        for ax, img, title, cmap in zip(axes[i], [gt_vol[i], recon_vol[i]],
                                        ["GT","Recon"], ["gray","gray"]):
...
class LitDCAE(pl.LightningModule):
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx, dataloader_idx=0):
        ...
        global_batch = local_batch * max(1, world_size)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if getattr(self, "global_rank", 0) == 0:
            wandb.log({"train/loss": float(loss.detach().cpu().item()), "train/psnr": float(psnr_val)}, step=getattr(self, "global_step", None))
        return loss
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=0):
        ...
        if step % self.save_every == 0 or step == cfg["num_steps"]:
            ...
            with torch.no_grad():
                ...
                gt = batch.float().cpu()
                l1_val = float(F.l1_loss(recon, gt).detach())
                ...
                if throughput is None:
                    if torch.cuda.is_available():
                        torch.cuda.synchronize()
                    t0 = time.time()
                    _ = pl_module(batch).float().cpu()
                    if torch.cuda.is_available():
                        torch.cuda.synchronize()
                    t1 = time.time()
                    step_time = max(1e-6, t1 - t0)
                    throughput = batch.shape[0] / step_time
                ...
                save_volume_slices_triplet((gt[0,0]*2000-1000).detach().numpy(), (recon[0,0]*2000-1000).detach().numpy(),
                                           os.path.join(step_dir,"triplet.png"))

                tqdm.write(f"Step {step} L1={l1_val:.2f} PSNR={psnr_val:.2f} dB Throughput={throughput:.2f} samples/s")
...
model = LitDCAE(cfg["model_name"], cfg["learning_rate"], cfg["weight_decay"])

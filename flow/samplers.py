...
def sample_velocity_model(
    ...
    if solver not in {"euler", "heun"}: raise ValueError(f"Unsupported solver: {solver}")
    print(f"[sampling] solver={solver} | steps={sample_steps} | schedule={time_schedule}")
    ...
    for step_idx, (t0, t1) in enumerate(zip(times[:-1], times[1:])):
        ...
        if progress_callback is not None: progress_callback(step_idx + 1, sample_steps - 1)
    return x

        ...
        for batch_2d in loader_2d:
            ...
            try:
                with open(log_file, "a") as f:
                    f.write(f"iter {global_step}: loss={loss.item():.6f}, recon={recon_loss.item():.6f}, perc={perc_loss.item():.6f}, PSNR={psnr_val:.6f}, SSIM={ssim_val:.6f}\n")
            except Exception as e:
                print(f"[WARNING] Failed to write to log file {log_file}: {e}")

            print(f"Iter {global_step}: loss={loss.item():.6f}, recon={recon_loss.item():.6f}, perc={perc_loss.item():.6f}, PSNR={psnr_val:.6f}, SSIM={ssim_val:.6f}")
